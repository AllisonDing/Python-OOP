...
class Buyer(Contact):
    ...
    def notify_when_remove(self, house):
        for h in self.watchList:
            if h == house:
                print(f"Dear buyer: {super().firstName} {super().lastName}, the following house of your watchlist has been removed:")
                h.display()
    #edit notifications to buyers
    def notify_when_edit(self, house):
        for h in self.watchList:
            if h == house:
                print(f"Dear buyer: {super().firstName} {super().lastName}, the following house of your watchlist has updated the price:")
                house.display()
...
class Company(Displayable):
    ...
    def notify_when_remove(self, house):
        ...
        #buyers
        for b in self.__buyers:
            b.notify_when_remove(house)
        #agents
        for a in self.__agents:
            a.notify_when_remove(house)

    def notify_when_edit(self, address):
        house = self.getHouseByAddress(address)
        #owner
        for o in self.__owners:
            o.notify_when_edit(house)
        #buyers
        for b in self.__buyers:
            b.notify_when_edit(house)
        #agents
        for a in self.__agents:
            a.notify_when_edit(house)
...
def main():
    ...
    company.notify_when_add(house1)
    company.notify_when_add(house2)
    company.notify_when_add(house3)
    
    
    agent1.editHousePrice('2222 Mission Blvd', 1200000)
    
    print('\nAfter one house was updated the price ..........................')
    company.notify_when_edit('2222 Mission Blvd')


    print('\nAfter one house was sold ..........................')
    company.notify_when_remove(house3)
